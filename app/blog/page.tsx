...
export const metadata: Metadata = {
  title: 'Blog | Cocinas Modernas: Diseños, Precios, Materiales y Tendencias en RD',
  description:
...
export const posts = [
  {
    slug: 'tendencias-cocinas-modernas-2025',
    title: 'Tendencias en cocinas modernas para 2025: colores, materiales e islas',
    excerpt:
      'Las cocinas modernas de 2025 combinan materiales naturales, gabinetes de cocina modernos en tonos neutros e islas de cocina modernas con desayunador. Descubre qué está marcando la pauta.',
  ...
  },
  {
    ...
    img: 'https://images.unsplash.com/photo-1600489000022-c2086d79f9d4?w=800&q=80',
    date: 'Enero 2025',
    readTime: '6 min',
    category: 'Diseño',
  },
  {
    ...
    title: 'Gabinetes de cocina modernos: madera, PVC, melamina o MDF',
    excerpt:
      'Gabinetes modernos en PVC, gabinetes de cocina modernos en melamina, madera o MDF. Comparamos materiales, precios y cuál es mejor para el clima de República Dominicana.',
  ...
  },
  {
    ...
    img: 'https://images.unsplash.com/photo-1588854337221-4cf9fa96059c?w=800&q=80',
    date: 'Noviembre 2024',
    readTime: '5 min',
    category: 'Diseño',
  },
  {
    slug: 'cuanto-cuesta-cocina-moderna-rd',
    title: '¿Cuánto cuesta una cocina moderna en República Dominicana en 2025?',
    excerpt:
  ...
  return (
    <>
      ...
      <section className="pt-36 pb-20 px-6 bg-obsidian-soft text-center relative">
        ...
        <p className="font-body text-xs tracking-[0.4em] uppercase text-gold mb-4">Guías y tendencias</p>
        <h1 className="font-display text-5xl md:text-6xl font-light text-cream mb-6">Blog de Cocinas Modernas</h1>
        <p className="font-body text-base text-cream/50 max-w-2xl mx-auto">
          Diseños de cocinas modernas pequeñas, gabinetes modernos, islas de cocina, precios, 
          materiales y colores de moda. Todo lo que necesitas saber para tu proyecto en RD.
        </p>
      </section>
      ...
      <section className="section-pad">
        <div className="max-w-7xl mx-auto">
          <Link href={`/blog/${featured.slug}`} className="group grid md:grid-cols-2 gap-0 luxury-card overflow-hidden border border-gold/10">
            ...
            <div className="bg-obsidian-soft p-10 md:p-14 flex flex-col justify-center">
              <div className="flex items-center gap-3 mb-6">
                ...
                <span className="font-body text-xs text-cream/30">{featured.date} · {featured.readTime}</span>
              </div>
              <h2 className="font-display text-3xl md:text-4xl font-light text-cream mb-4 group-hover:text-gold transition-colors">
                {featured.title}
              </h2>
              <p className="font-body text-sm text-cream/60 leading-relaxed mb-8">{featured.excerpt}</p>
              <span className="font-body text-xs tracking-widest uppercase text-gold flex items-center gap-2">
                Leer artículo <span className="group-hover:translate-x-2 transition-transform inline-block">→</span>
              </span>
            </div>
          </Link>
        </div>
      ...
      <section className="pb-24 px-6 md:px-12 lg:px-24">
        <div className="max-w-7xl mx-auto grid md:grid-cols-2 lg:grid-cols-3 gap-8">
          {rest.map((post) => (
            <Link key={post.slug} href={`/blog/${post.slug}`} className="luxury-card group border border-gold/10 bg-obsidian-soft overflow-hidden">
              ...
              <div className="p-6">
                <div className="flex items-center gap-3 mb-4">
                  ...
                  <span className="font-body text-xs text-cream/30">{post.readTime}</span>
                </div>
                <h3 className="font-display text-2xl text-cream mb-3 group-hover:text-gold transition-colors">
                  {post.title}
                </h3>
                <p className="font-body text-sm text-cream/50 leading-relaxed">{post.excerpt}</p>
                <div className="gold-line mt-6 mb-4" />
                <span className="font-body text-xs tracking-widest uppercase text-gold/60">{post.date}</span>
              </div>
            </Link>
          ))}
        </div>
